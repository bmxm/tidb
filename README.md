...
乍一看，TiDB有80个包，这可能会让你感到不知所措，但并不是所有的包都很重要，有些功能只涉及少量的包，所以从哪里开始查看源代码取决于查看源代码的目的。
如果您想了解特定功能的实现细节，那么可以参考上面的模块描述，找到相应的模块。
如果想全面了解源代码，可以从tidb server/main开始。去看看tidb服务器是如何启动的，以及它是如何等待和处理用户请求的。
然后一直按照代码来查看SQL的确切执行情况。还有一些重要的模块需要查看，以了解它们是如何实现的。对于辅助模块，您可以有选择地查看它们，以获得总体印象。
